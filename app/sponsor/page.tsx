        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {children.map((child, index) => (
              <div 
                ...
                className={`bg-dark-200 rounded-2xl overflow-hidden border transition-all duration-300 cursor-pointer ${
                  selectedChild === index ? 'border-primary-500 ring-2 ring-primary-500/50' : 'border-black/10 hover:border-primary-500/50'
                }`}
              ...
              >
                <div className="relative h-48 bg-gradient-to-br from-primary-500/20 to-primary-600/10 flex items-center justify-center">
                  <div className="w-24 h-24 rounded-full bg-black/10 backdrop-blur-sm flex items-center justify-center">
                    <span className="text-4xl font-bold text-black/60">{child.name[0]}</span>
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="text-xl font-bold text-black mb-1">{child.name}, {child.age}</h3>
                  ...
                  <p className="text-black/50 text-sm mb-4">{child.story}</p>
          ...
          {selectedChild !== null && (
            <div className="max-w-2xl mx-auto">
              <div className="bg-dark-200 rounded-2xl p-8 border border-black/10">
                <h3 className="text-2xl font-bold text-black mb-2 text-center">Sponsor {children[selectedChild].name}</h3>
                <p className="text-black/40 text-center mb-6">Monthly sponsorship of ₦{children[selectedChild].monthlyCost.toLocaleString()}</p>
                ...
                <form className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-black/60 text-sm mb-2">Full Name</label>
                      <input type="text" className="w-full bg-black/10 border border-black/20 rounded-xl px-4 py-3 text-black" />
                    </div>
                    <div>
                      <label className="block text-black/60 text-sm mb-2">Email</label>
                      <input type="email" className="w-full bg-black/10 border border-black/20 rounded-xl px-4 py-3 text-black" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-black/60 text-sm mb-2">Phone Number</label>
                    <input type="tel" className="w-full bg-black/10 border border-black/20 rounded-xl px-4 py-3 text-black" />
                  </div>
                  <div>
                    <label className="block text-black/60 text-sm mb-2">Address</label>
                    <input type="text" className="w-full bg-black/10 border border-black/20 rounded-xl px-4 py-3 text-black" />
                  </div>
                  <button className="w-full bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-black py-3 rounded-xl font-semibold transition-all duration-300">
          ...
          {selectedChild === null && (
            <div className="text-center">
              <p className="text-black/50 mb-4">Click on a child above to start sponsoring</p>
            </div>
          )}
        </div>
      </section>
