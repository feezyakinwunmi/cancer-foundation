        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
            {[
              ...
              <div key={index} className="text-center p-4 bg-black/5 rounded-xl border border-black/10">
                <stat.icon className="w-8 h-8 text-primary-400 mx-auto mb-2" />
                <p className="text-2xl font-bold text-black">{stat.number}</p>
                <p className="text-black/40 text-sm">{stat.label}</p>
              </div>
            ))}
          </div>

          {/* Opportunities Grid */}
          <h3 className="text-2xl font-bold text-black mb-6 text-center">Volunteer Opportunities</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
            {opportunities.map((opp, index) => (
              <div key={index} className="bg-dark-200 rounded-2xl p-6 border border-black/10 hover:border-primary-500/30 transition-all duration-300">
                ...
                <h4 className="text-xl font-bold text-black mb-2">{opp.title}</h4>
                <p className="text-black/50 text-sm mb-3">{opp.description}</p>
                <div className="flex flex-wrap gap-3 mb-4">
                  <span className="text-xs text-black/40 bg-black/5 px-2 py-1 rounded-full">⏱ {opp.commitment}</span>
                  <span className="text-xs text-black/40 bg-black/5 px-2 py-1 rounded-full">📚 {opp.skills}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-dark-200 rounded-2xl p-8 border border-black/10">
              <h3 className="text-2xl font-bold text-black mb-6 text-center">Apply to Volunteer</h3>
              <form className="space-y-4">
                <div>
                  <label className="block text-black/60 text-sm mb-2">Full Name</label>
                  <input
                    type="text"
                    className="w-full bg-black/10 border border-black/20 rounded-xl px-4 py-3 text-black focus:outline-none focus:border-primary-500"
                ...
                <div>
                  <label className="block text-black/60 text-sm mb-2">Area of Interest</label>
                  <select className="w-full bg-black/10 border border-black/20 rounded-xl px-4 py-3 text-black focus:outline-none focus:border-primary-500">
                ...
                <div>
                  <label className="block text-black/60 text-sm mb-2">Why do you want to volunteer?</label>
                  <textarea
                    rows={4}
                    className="w-full bg-black/10 border border-black/20 rounded-xl px-4 py-3 text-black focus:outline-none focus:border-primary-500"
